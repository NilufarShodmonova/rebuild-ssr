...
window.bdor||((this||(0,eval)("(this)")).bdor=[]);window.bdor[1]="p";var global=function(){return"undefined"!==typeof globalThis?globalThis:"undefined"!==typeof window?window:"undefined"!==typeof global?global:this}(),virtual_function=function(){};function getPackageByName(a){if(void 0==a||""==a)return global;var b=global;a=a.split(".");for(var c=0;c<a.length;c++){var d=a[c];b[d]||(b[d]={IS_PACKAGE_OBJECT:!0});b=b[d]}return b}
function getClassByFullName(a){if(-1==a.indexOf("."))return global[a];var b=a.split(".");a=b.pop();b=b.join(".");return getPackageByName(b)[a]}
...
this.prePage.hide()},setQuestionURL:function(a){this.content.append(a)},initEvent:function(){var a=this;this.currentIndex=0;var b=this.topicItem.length;this.OnNextPageClick=function(){if(!(this.currentIndex>b))if(this.currentIndex+=1,this.progressBar.setProgress(this.currentIndex+1),this.currentIndex<b)this.setQuestionURL(this.topicItem[this.currentIndex].QuizTopicPanel.content),this.topicItem[this.currentIndex].QuizTopicPanel.Animate(),this.topicItem[this.currentIndex].resetScroll(),this.prePage.show();
else{var c=new Onclusion(this,this.config.action,this.rightNumber[0].innerText,this.mistakeNumber[0].innerText);this.setQuestionURL(c.QuizTopicPanel.content);this.nextPage.hide()}}.bind(this);this.nextPage.on("click",this.OnNextPageClick);this.prePage.on("click",function(){0!=this.currentIndex&&(0==this.currentIndex&&(this.currentIndex+=b),this.currentIndex=(this.currentIndex-1)%b,this.setQuestionURL(this.topicItem[this.currentIndex].QuizTopicPanel.content),this.topicItem[this.currentIndex].QuizTopicPanel.Animate(),
this.progressBar.setProgress(this.currentIndex+1),this.nextPage.show(),0==this.currentIndex&&this.prePage.hide())}.bind(this));this.closeButton.bind("click",function(c){c.stopPropagation();this.hide()}.bind(this));this.nextPage.on("mouseover",function(){this.nextPage.css({background:"url("+uiBaseURL+"quiz.png) no-repeat -257px -61px",cursor:"pointer"})}.bind(this));this.nextPage.on("mouseout",function(){this.nextPage.css("background","url("+uiBaseURL+"quiz.png) no-repeat -257px -16px")}.bind(this));
this.prePage.on("mouseover",function(){this.prePage.css({background:"url("+uiBaseURL+"quiz.png) no-repeat -310px -62px",cursor:"pointer"})}.bind(this));this.prePage.on("mouseout",function(){this.prePage.css("background","url("+uiBaseURL+"quiz.png) no-repeat -310px -17px")}.bind(this));$(window).on("resize",function(){return a.onResize()})},initQuiz:function(){this.topicItem=[];this.topicAnswer=[];for(var a=0;a<this.quiz.length;a++)"right"==this.config.action.languageLayout&&(this.quiz[a].alignCss=
{"text-align":"right",direction:"rtl"}),this.topic=new SetQuestionPanel(this,this.quiz[a],a),this.topicItem.push(this.topic),this.topicAnswer[a]=null},getAnswerFlags:function(){for(var a=[],b=0;b<this.quiz.length;b++)null===this.topicAnswer[b]?a[b]=QAF_UNKNOWN:this.topicAnswer[b]==this.topicItem[b].getRightAnswer()?a[b]=QAF_RIGHT:a[b]=QAF_MISTAKE;return a},statistics:function(){for(var a=this.getAnswerFlags(),b=0,c=0,d=0;d<a.length;d++)a[d]==QAF_RIGHT&&b++,a[d]==QAF_MISTAKE&&c++;this.rightNumber[0].innerText=
Number(b).toString();this.mistakeNumber[0].innerText=Number(c).toString()},showButton:function(){this.rightSmile.show();this.mistakeSmile.show();this.rightNumber.show();this.mistakeNumber.show();this.nextPage.show();this.prePage.show()},hideButton:function(){this.rightSmile.hide();this.mistakeSmile.hide();this.rightNumber.hide();this.mistakeNumber.hide();this.nextPage.hide();this.prePage.hide()},setCss:function(){this.title.css({position:"relative",margin:"auto",width:"570px",height:"25px","margin-top":"20px",
"text-align":"center","font-size":"20px","font-family":"-webkit-pictograph"});this.closeButton.asButton();this.closeButton.css({position:"absolute","background-image":"url("+uiBaseURL+"close_form.png)",width:"32px",height:"32px","z-index":"500",cursor:"pointer",right:"30px","margin-top":"24px"});this.dialog.css({position:"absolute",width:"760px",height:"550px",margin:"auto",left:"0",top:"0",right:"0",bottom:"0","z-index":"1001"});this.nextPage.css({background:"url("+uiBaseURL+"quiz.png) no-repeat -257px -16px",
position:"absolute",margin:"auto",right:"22px",top:"210px",width:"36px",height:"36px"});this.prePage.css({background:"url("+uiBaseURL+"quiz.png) no-repeat -310px -17px",position:"absolute",margin:"auto",left:"27px",top:"210px",width:"36px",height:"36px"});this.content.css({position:"absolute",background:"url("+uiBaseURL+"quiz.png) no-repeat -6px -148px","max-width":"722px","max-height":"502px",width:"100%",height:"100%",margin:"auto",left:"0",top:"0",right:"0",bottom:"0"})},onResize:function(){},
...
Class("PCProductDetailsPanel",{Package:"PageItem",create:function(a,b,c){var d=this;this.advancedObj=new PageItem.ShoppingAdvancedProperty(a);this._super(a,b,c);this.shoppingCart||(this.shoppingCart=new PageItem.PCShoppingCartPanel(a,b),window.shoppingCart=this.shoppingCart);this.shoppingCart.onClose=function(){return d.shoppingCartClose()};shoppingCountHTML.html(this.shoppingCart.refreshInputValue());this.canCheckout=!0;this.visible=!1},parseConfig:function(a){return{annotype:a.annotype,x:a.location.x,
y:a.location.y,width:a.location.width,height:a.location.height,bgColor:a.bgColor,bgAlpha:a.bgAlpha,cartType:a.cartType,imgUrl:a.imgUrl,titleStr:a.titleStr,url:a.url,languageLayout:a.languageLayout,Detailed:a.Detailed}},destroy:function(){this.container.empty();this.container.remove()},initUI:function(){var a=this;this.initHtml();this.setShopCss();tmpContainer.append(this.bgImage);tmpContainer.append(this.container);this.container.append(this.content);this.container.append(this.closeButton);this.content.append(this.imgBox);
...
[];for(var a=0;a<this.imgs.length;a++)this.imageItems.push(this.imgs[a]),bookConfig.isFlipPdf&&this.imageItems[a]&&(this.imageItems[a].startsWith("./")?this.imageItems[a]="."+this.imageItems[a]:this.imageItems[a].isUrl()||(this.imageItems[a]="../"+this.imageItems[a]));1<this.imgs.length&&(this.imageURL=this.imageItems[0],this.initButton());this.setImgs(this.imageItems[0])}},setImgs:function(a){this.shopImg.attr("src",a)},initButton:function(){var a=this;this.leftButton=$('<img src="'+uiBaseURL+'photo_prev.png" style="position:absolute;z-index:1000;cursor:pointer;opacity:0.8;"></img>');
this.rightButton=$('<img src="'+uiBaseURL+'photo_next.png" style="position:absolute;z-index:1000;cursor:pointer;opacity:0.8;"></img>');this.setButtonPosition();this.currentIndex=0;var b=this.imageItems.length;this.leftButton.on(_event._click,function(){a.busy=!0;0==a.currentIndex&&(a.currentIndex+=b);a.currentIndex=(a.currentIndex-1)%b;a.imageURL=a.imageItems[a.currentIndex];a.setImgs(a.imageURL)});this.rightButton.on(_event._click,function(){a.busy=!0;a.currentIndex=(a.currentIndex+1)%b;a.imageURL=
a.imageItems[a.currentIndex];a.setImgs(a.imageURL)});this.imgBox.append(this.leftButton).append(this.rightButton)},setButtonPosition:function(){this.leftButton.css({left:"17px",top:"143.5px"});this.rightButton.css({right:"17px",top:"143.5px"})},setShopCss:function(){this.bgImage.css({position:"absolute",width:windowWidth+"px",height:windowHeight+"px",opacity:"0.5",top:0,"background-color":"black","z-index":"100"});var a="translate(0, "+-((windowHeight-430)/2+430)+"px)";this.container.css({position:"absolute",
width:"730px",height:"430px",margin:"auto",left:"0",top:"0",bottom:"0",right:"0",transform:a,"-webkit-transform":a,"-moz-transform":a,"-o-transform":a,"-ms-transform":a,"z-index":"2000"});this.content.css({position:"absolute",width:"700",height:"400",margin:"auto",left:"0",top:"0",bottom:"0",right:"0","background-color":"white","z-index":"2000"});this.closeButton.css({position:"absolute",width:"20px",height:"20px",right:"0","background-image":"url("+uiBaseURL+"closecat.png)","background-repeat":"no-repeat",
cursor:"pointer"});this.imgBox.css({"background-color":"#f4f4f4",position:"absolute",width:"330px",height:"330px",margin:"auto",left:"20px",top:"0",bottom:"20px"});this.shopImg.css({"max-width":"300px","max-height":"300px",position:"absolute",margin:"auto",left:"0",top:"0",bottom:"0",right:"0"});this.contentText.css({position:"absolute",width:"304px",height:"355px",margin:"auto","margin-top":"30px",right:"20px","overflow-x":"hidden","-webkit-user-select":"initial","-moz-user-select":"initial","-ms-user-select":"initial",
"user-select":"initial"});this.shopTitle.css({height:"30px","font-size":"30px",color:"black","font-family":"Arial"});this.about.css({height:"148px","margin-top":"10px","font-size":"10px","font-family":"Arial","overflow-x":"hidden"});this.quantityBox.css({height:"20px","line-height":"17px","margin-top":"10px",color:"black","font-size":"10px","font-family":"Arial"});this.number.css({});this.time.css({height:"14px","margin-top":"10px",color:"black","font-size":"10px","font-family":"Arial"});this.allQuantity.css({height:"14px",
"margin-top":"8px","font-size":"10px",color:Color(44849).toString(),"font-family":"Arial"});this.shopPrice.css({height:"18px","margin-top":"10px","font-size":"16px",color:Color(12845056).toString(),"font-family":"Arial"});this.addToCart.css({background:"rgb(80, 80, 80)","text-align":"center","line-height":"2.0","font-size":"10px","font-family":"Arial",float:"left",padding:"0 5px","margin-top":"25px","margin-left":"0",cursor:"pointer",color:"white"});this.checkout.css({background:"rgb(80, 80, 80)",
...
this.add.css({float:"right"}),this.low.css({float:"right"}),this.checkout.css({left:"158px",right:"auto"}),this.close.css({left:"100px",right:"auto"}))},initEvent:function(){var a=this;this.input.on("blur",function(){parseInt(a.input.val())>a.config.Detailed.quantity?(a.number.append(a.not),a.input.css({"border-color":"red"}),a.canCheckout=!1):(a.not.remove(),a.input.css({"border-color":"rgb(150,150,150)"}),a.canCheckout=!0)});this.add.on(_event._click,function(){var b=a.input.val();b=parseInt(b)+
1;a.input.val(b);b>a.config.Detailed.quantity?(a.number.append(a.not),a.input.css({"border-color":"red"}),a.canCheckout=!1):a.canCheckout=!0;ShoppingCartUtil.saveProducts(a.shoppingItems,a.setting)});this.low.on(_event._click,function(){var b=parseInt(a.input.val());0!=b&&(b=1>b-1?1:b-1,b<=a.config.Detailed.quantity&&(a.not.remove(),a.input.css({"border-color":"rgb(150,150,150)"}),a.canCheckout=!0),a.input.val(b))});this.mailto.on(_event._click,function(){bookConfig.googleAnalyticsID&&gtag("event",
getEditorEventName()+"click",{event_category:"mailtoCheckOut",event_label:"doCheckOut"});a.doCheckout()});this.bgImage.on(_event._click,function(){a.hide();a.shoppingCart.cart.css({display:"none"})});this.closeButton.on(_event._click,function(){return a.hide()});this.close.on(_event._click,function(){return a.hide()});this.addToCart.on(_event._click,function(){bookContainer.append(a.cart);a.bgImage.css({display:"none"});a.hide();a.shoppingCart.showCart();var b={};b.image=a.imageItems&&1>=a.imageItems.length?
a.imageItems[0]:a.imageURL;b.advancedName=a.advancedObj.setShoppingCartAdvanced().toString();b.title=a.config.Detailed.title;b.sing=a.config.Detailed.sing;b.price=parseInt(a.config.Detailed.proce);b.number=parseInt(a.input.val());b.quantity=parseInt(a.config.Detailed.quantity);a.shoppingCart.addProduct(b);shoppingCountHTML.html(a.shoppingCart.refreshInputValue())})},shoppingCartClose:function(){this.bgImage.css({display:"none"})},onResize:function(){var a=this.container.css("width"),b=this.container.css("height");
this.height=parseInt(b.replace("px",""));this.width=parseInt(a.replace("px",""));this.container.css({"margin-left":(windowWidth-this.width)/2+"px","margin-top":(windowHeight-this.height)/2+"px"});this.bgImage.css({width:windowWidth+"px",height:windowHeight+"px"})},skipHeid:function(){var a=this.container.css("height");this.height=parseInt(a.replace("px",""));a="translate(0, "+-((windowHeight-this.height)/2+this.height)+"px)";a={transform:a,"-webkit-transform":a,"-moz-transform":a,"-o-transform":a,
"-ms-transform":a};this.bgImage.css({display:"none"});animateOnce(this.container,a,.5,null,"","ease-in-out")},hide:function(){if(!this.running){this.running=!0;var a=this.container.css("height");this.height=parseInt(a.replace("px",""));a="translate(0, "+-((windowHeight-this.height)/2+this.height)+"px)";a={transform:a,"-webkit-transform":a,"-moz-transform":a,"-o-transform":a,"-ms-transform":a};(function(){this.container.css({display:"none"});this.content.css("display","none");this.bgImage.css({display:"none"});
this.running=!1}).delay(this,500);animateOnce(this.container,a,.5,null,"","ease-in-out")}},show:function(){var a=this;this.running||(this.running=!0,this.bgImage.css({display:"block"}),this.changeLanguage(),this.content.css("display","block"),this.container.css({display:"block"}),animateOnce.delay(this,10,[this.container,{transform:"translate(0px,0px)","-webkit-transform":"translate(0px,0px)","-moz-transform":"translate(0px,0px)","-o-transform":"translate(0px,0px)","-ms-transform":"translate(0px,0px)"},
.5,function(){a.running=!1},"","ease-in-out"]))}}).extend(PageItem.ProductDetailsPanel);
...
this.canCheckout=!1},initFlipPdfCase:function(){this.cartTitles=$("<span>\u8d2d\u7269\u8f66</span>");this.cartprices.html("\u4ef7\u683c");this.cartqu.html("\u6570\u91cf");this.cartCheckout=$("<span>\u4ed8\u6b3e</span>");this.cartClose=$("<span>\u5173\u95ed</span>");this.cartAllDelete=$("<span>\u5220\u9664\u6240\u6709</span>");this.deleteBox=$('<div>\u786e\u5b9a\u5220\u9664\u6240\u6709\u5546\u54c1\u5417? <br/><span class="yes">\u662f</span><span class="no">\u5426</span></div>')},initEvent:function(){var a=
this;this.bgImage.on(_event._end,function(){a.cart.css({display:"none"});a.bgImage.css({display:"none"})});this.deleteBox.find(".yes").on(_event._click,function(){a.deleteAll();a.deleteBox.hide(300)});this.deleteBox.find(".no").on(_event._click,function(){a.deleteBox.hide(300)});this.cartAllDelete.on(_event._click,function(){a.deleteBox.show(300)});this.cartMailto.on(_event._click,function(){bookConfig.googleAnalyticsID&&gtag("event",getEditorEventName()+"click",{event_category:"ShoppingCart",event_label:"CheckOutAll"});
a.doCheckout()});this.cartClose.on(_event._click,function(){a.hideCart();if(a.onClose)a.onClose();ShoppingCartUtil.saveProducts(a.shoppingItems,a.setting)});$(window).on("resize",function(){a.bgImage.css({width:windowWidth+"px",height:windowHeight+"px"})})},deleteAll:function(){for(ShoppingCartUtil.setProductsToStorage([]);0<this.shoppingItems.length;)this.removeItem(this.shoppingItems[0]),this.refreshTotalValue()},changeLanguage:function(){this.cartTitles.html(getLanguage("TStuffCart_TypeCart","Shopping Cart"));
this.cartprices.html(getLanguage("TStuffCart_DetailedPrice","Price"));this.cartqu.html(getLanguage("TStuffCart_DetailedQuantity","Quantity"));this.cartCheckout.html(getLanguage("ShappingCart_CheckOut","Checkout"));this.cartClose.html(getLanguage("ShappingCart_Close","Close"));this.cartAllDelete.html(getLanguage("lblRemoveAll","Remove All"));this.deleteBox.find(".deleteDetial").html(getLanguage("lblRemoveAll","Remove All")+" "+getLanguage("ShappingCart_Item","Items")+"?");this.deleteBox.find(".yes").html(getLanguage("lblDelete",
...
getLanguage("ShappingCart_NotStock","Not enough in stock")+"</span>");this.singleShop.append(this.shoppingImage);this.singleShop.append(this.carttitle);this.singleShop.append(this.addlow);this.singleShop.append(this.cartPrice);this.singleShop.append(this.dustbin);this.addlow.append(this.cartadd);this.addlow.append(this.cartlow);this.addlow.append(this.cartInput);this.setImg(this.product.image)},setImg:function(a){this.shoppingImage.attr("src",a)},initEvent:function(){var a=this;this.cartInput.on("keyup",
function(){var b=parseInt(a.cartInput.val());""==b&&(b=a.cartInput.val());b>a.product.quantity?(a.addlow.append(a.cartNot),a.cartInput.css({"border-color":"red"}),a.canCheckout=!1):(a.cartNot.remove(),a.cartInput.css({"border-color":"rgb(150,150,150)"}),a.canCheckout=!0);a.product.number=b;a.shop.refreshTotalValue();ShoppingCartUtil.saveProducts(a.shop.shoppingItems,a.shop.setting);shoppingCountHTML.html(a.shop.refreshInputValue())});this.dustbin.on(_event._click,function(){a.shop.removeItem(a);a.shop.refreshTotalValue();
shoppingCountHTML.html(a.shop.refreshInputValue())});this.cartadd.on(_event._click,function(){var b=a.cartInput.val();b=parseInt(b)+1;a.cartInput.val(b);a.product.number=b;b>a.product.quantity&&(a.addlow.append(a.cartNot),a.cartInput.css({"border-color":"red"}));a.shop.refreshTotalValue();ShoppingCartUtil.saveProducts(a.shop.shoppingItems,a.shop.setting);shoppingCountHTML.html(a.shop.refreshInputValue())});this.cartlow.on(_event._click,function(){var b=a.cartInput.val();0!=parseInt(b)&&(b=1>parseInt(b)-
1?1:parseInt(b)-1,a.cartInput.val(b),a.product.number=b,b<=a.product.quantity&&(a.cartNot.remove(),a.cartInput.css({"border-color":"rgb(150,150,150)"})),a.shop.refreshTotalValue(),ShoppingCartUtil.saveProducts(a.shop.shoppingItems,a.shop.setting),shoppingCountHTML.html(a.shop.refreshInputValue()))})},setCartCSS:function(){this.cartPrice.css({position:"absolute",width:"70px",height:"20px","font-size":"12px",color:"black","margin-top":"30px","font-family":"Tahoma",right:"65px"});this.cartInput.css({"text-align":"center",
width:"40px",height:"16px",outline:"medium","margin-left":"50px",position:"absolute",background:"#fff",border:"1px solid rgb(150,150,150)"});this.addlow.css({width:"120px",height:"40px",right:"170px",position:"absolute","margin-top":"35px"});this.shoppingImage.css({position:"absolute",width:"52px",height:"35px","margin-top":"28px",left:"10px","-moz-box-shadow":"0 0 5px #fff","-o-box-shadow":"0 0 5px #fff","-webkit-box-shadow":"0 0 5px #fff","box-shadow":"0 0 5px #fff"})}}).extend(PageItem.ShoppingCartItem);
